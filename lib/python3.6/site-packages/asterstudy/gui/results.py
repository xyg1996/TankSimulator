...
class Results(Q.QWidget):
    ...
    def __init__(self, astergui, parent=None):
        ...
        self.sidebar.pushButton_3.hide()
        self.sidebar.pushButton_4.hide()
        ...
        self.res_splitter.setMinimumWidth(300)
    ...
    def load_ofccx_result_call(self,path=None):
        ...
        print(self.fluidfoam)

        # set active source
        pvs.SetActiveSource(self.fluidfoam)
        self.fluidfoam.MeshRegions = ['internalMesh']

        for array in self.fluidfoam.CellArrays:
            print("array:",array)
            self.sidebar.comboBox_7.addItem(array)

        foam_array = ["alpha.water","U","p"]
        for array in foam_array:
            self.sidebar.comboBox_7.addItem(array)

        pvs.ColorBy(self.currentdisplay1, ('CELLS', "alpha.water"))
        self.colorobject["alpha.water"] = pvs.GetColorTransferFunction("alpha.water")

        # for array in self.fluidfoam.PointArrays:
        #     print("fluid_array:",array)
        #     self.sidebar.comboBox_7.addItem(array)
        ...
        connect(self.sidebar.comboBox_8.activated, self.renviewdirchange1)
    ...
    def renviewchange1(self):
        arrayname = self.sidebar.comboBox_7.currentText()
        self.sidebar.comboBox_8.clear()
        if arrayname in ['U']:
            directs = ['Magnitude','X','Y','Z']
        else:
            directs = []
        for d in directs:
            self.sidebar.comboBox_8.addItem(d)

        # pvs.ColorBy(self.currentdisplay1, ('POINTS', arrayname))
        pvs.ColorBy(self.currentdisplay1, ('CELLS', arrayname))
        self.colorobject[arrayname] = pvs.GetColorTransferFunction(arrayname)
        # pvs.Render()
        self.updateview()

    def renviewdirchange1(self):
        dir = self.sidebar.comboBox_8.currentText()
        arrayname = self.sidebar.comboBox_7.currentText()
        pvs.ColorBy(self.currentdisplay, ('CELLS', arrayname, dir))
        self.colorobject[arrayname] = pvs.GetColorTransferFunction(arrayname)
        self.updateview()
