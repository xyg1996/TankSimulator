...
class Maincontrol(Q.QWidget):
    ...
    def addcase(self):
        ...
        if not BOUNDARY or not medfilename:
            ...
            # src = p.template_dir
            src = "/share/simforge_share/open-source/Tanksimulator/Tanksimulator/template"
            print("template source:",src)
            ...
            param_fluid = dict(coordrang = [xmin,xmax,ymin,ymax,zmin,zmax],
                               ...
                               cpus = self.mpicores.currentText()
                               )
    ...
    def showreact(self):
            ...
            if sender == self.btFluidpre:
                ...
                self.key = Key
                cmdfluidpre = '/share/simforge_share/open-source/Tanksimulator/Tanksimulator/SALOME-9.4.0-CO7-SRC/sub.fluidrun' + ' ' + \
                    self.currentpath + ' ' + self.mpicores.currentText() + ' ' + str(MaxT1) + ' ' + str(MaxT2) + ' ' + str(self.key)
            ...
            if sender == self.bt1:
                ...
                if not self.currentpath or not os.path.exists(self.currentpath):
                    ...
                    cmdsolid = "/share/simforge_share/open-source/Tanksimulator/Tanksimulator/SALOME-9.4.0-CO7-SRC/sub.solidrun" + " " + self.currentpath \
                    + ' ' + str(self.key )
            ...
            if sender == self.bt4:
                ...
                docx_path = os.path.join(self.currentpath,"demo.docx")
                TempOutput = self.temptextview.textCursor()
                if os.path.exists(docx_path):
                    TempOutput.insertText("报告已存在, 跳过生成。即将打开......")
                else:
                    self.tempprocess.start("/share/simforge_share/open-source/Tanksimulator/Tanksimulator/\
                                           SALOME-9.4.0-CO7-SRC/BINARIES-CO7/ASTERSTUDY/lib/python3.6/site-packages/asterstudy/gui/hexinjisuan/WordShell "+self.currentpath) #&& /usr/bin/libreoffice demo.docx")
            ...
            if sender == self.btbothkill:
                ...
                self.process.start('/opt/skyformai/bin/ckill -J Tanksimulator_solidrun')
                self.processfluid.start('/opt/skyformai/bin/ckill -J Tanksimulator_fluidrun')
